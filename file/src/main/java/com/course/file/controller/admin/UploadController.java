...
//        String path = dir + File.separator + key + "." + suffix + '.' + fileDto.getShardIndex();
    ...
    public ResponseDto merge() throws Exception {
        ...
        try {
            //读取第一个分片
            fileInputStream = new FileInputStream(new File(FILE_PATH + "/course/nEvzwt2t.blob"));
            while ((len = fileInputStream.read(byt)) != -1){
                outputStream.write(byt, 0, len);
            }
            //读取第二个分片
            fileInputStream = new FileInputStream(new File(FILE_PATH + "/course/WRUrP6Dv.blob"));
            while ((len = fileInputStream.read(byt)) != -1){
                outputStream.write(byt, 0, len);
            }
        } catch (IOException e) {
